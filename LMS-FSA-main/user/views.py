...
def export_users(request):
    ...
    for user in User.objects.all():
        # Handle cases where the user might not have an associated role
        role_id = user.role.id if user.role else 'N/A'
        role_name = str(user.role) if user.role else 'No Role Assigned'
        worksheet.append([user.username, '******', user.email, user.full_name, role_id, role_name])
